...
export class StepDetailViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    // Clear stale reference on dispose
    webviewView.onDidDispose(() => {
      log('debug', 'StepDetailViewProvider: webview disposed');
      this.view = undefined;
    });

    // Re-render when the view becomes visible again
    webviewView.onDidChangeVisibility(() => {
      log('debug', 'StepDetailViewProvider: visibility changed', { visible: webviewView.visible });
